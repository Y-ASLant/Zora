#!/usr/bin/env bash
# 在远端主机安装 Zora CLI 二进制,用于 remote-server-proxy。
#
...
if [ ! -f "$bin" ]; then
  bin=$(find "$tmpdir" -type f \( -name 'zora' -o -name 'zap-oss' -o -name 'warp-oss' -o -name 'oz*' \) ! -path "$tmpdir/resources/*" ! -name '*.tar.gz' | head -n1)
fi
