# TECH.md — Rename Oz to Zora Agent in settings and onboarding
...
This is a rename of the in-app agent from "Oz" to "Zora Agent" across user-facing
...
1. `app/src/settings_view/mod.rs`
   ...
   - In the `Display` impl, the `WarpAgent` arm writes `"Zora Agent"`.
   - In the `FromStr` impl, accept both `"Oz"` (backward-compat legacy name)
     and `"Zora Agent"` as parseable forms that map to
...
2. `app/src/settings_view/ai_page.rs`
   ...
   - In `GlobalAIWidget::render`, replace `Text::new_inline("Oz", ...)` with
     `Text::new_inline("Zora Agent", ...)`. Keep every other argument, style,
   ...
   - Replace all remaining user-visible description strings that referenced
     "Oz" or "Oz agent" with "the Zora Agent" / "Zora Agent" as appropriate.
     ...
     allowlist/denylist descriptions, Rules description, Zora Drive context
...
3. `crates/onboarding/src/slides/agent_slide.rs`
   - `render_header`: change paragraph text from `"Customize your Agent, Oz"`
     to `"Customize your Zora Agent"`. Keep font size, weight, layout, and
     surrounding subtitle unchanged.
   - `render_disable_oz_section`: change checkbox label from `"Disable Oz"` to
     `"Disable Zora Agent"`. Keep styling, spacing, `disable_oz_mouse` state
...
- `cargo nextest run -p warp_app --no-fail-fast` or the relevant subset covering
  ...
  `SettingsSection::WarpAgent.to_string() == "Zora Agent"`. The `FromStr` test
  covers both `"Oz"` and `"Zora Agent"` resolving to
...
- Behavior #1, #2, #3, #9: manually open the settings UI and confirm the
  sidebar entry reads "Zora Agent", the subpage renders unchanged content, the
  heading above the global toggle reads "Zora Agent", and the "Oz Cloud API
...
- Behavior #8: confirm both `"Oz"` and `"Zora Agent"` resolve to
...
- Behavior #12: toggle the `ZapNewSettingsModes` feature flag and confirm
  the disable row only appears when enabled and always reads "Disable Zora
...
- Risk: external deep links or persisted telemetry strings reference `"Oz"` and
  break. Mitigation: `FromStr` accepts both `"Oz"` and `"Zora Agent"` mapping to
