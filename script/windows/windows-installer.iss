...
#define MyAppPublisher "Zora"
  ...
  ; 默认跟随官方 channel 的 `dev.warp.*` 命名;OSS 在 bundle.ps1 里会覆盖为 `dev.warp.zora`。
...
; Force close previous Zora if it hasn't shut down yet.
; In the update flow we already warn the user if they have something running and make them confirm
; before running this installer. Therefore, we are good to force close Zora without fear of losing
; unsaved work.
...
; For manual installs: if Zora is running, show a dialog prompting the user to close it
...
; The directive was added in Inno Setup 6.7. Older compilers do not implement
; the guard, so omitting it there preserves their existing behavior.
...
[Registry]
Root: HKCU; Subkey: "dev.warp.zora\{#MyAppName}"; Flags: uninsdeletekey
; cleanup "Open Zora Here" registry entries
...
; Add "Open Zora in new tab" to directory context menu
...
; Add "Open Zora in new tab" to directory background context menu
...
; Add "Open Zora in new window" to directory context menu
...
; Add "Open Zora in new window" to directory background context menu
...
[Tasks]
Name: addToPath; Description: "Add Zora to PATH"

[UninstallDelete]
Type: filesandordirs; Name: "{userappdata}\warp\zora"
Type: filesandordirs; Name: "{localappdata}\warp\zora"
...
{ Returns true when the installer was launched by Zora's auto-update code.
  The auto-update path passes /update=1 on the command line and /NOCLOSEAPPLICATIONS
  so that the installer does not forcibly kill the running Zora process. Instead we
  wait for Zora to exit naturally by polling the app mutex below. }
...
{ For background updates, return an empty mutex name so that Inno Setup skips its
  ...
  installs, return the real mutex name so the user is prompted to close Zora first. }
...
var
  ...
  ResultCode: Integer;
begin
  { Background update: the installer was launched while Zora was still running.
    We passed /NOCLOSEAPPLICATIONS so Inno won't kill it. Wait here - before any
    files are touched - for Zora to release its single-instance mutex, which
    happens as part of normal process exit. }
  if CurStep = ssInstall then
  begin
    if IsBackgroundUpdate() then
    begin
      Log('Background update: waiting for Zora to exit (mutex: {#AppMutexName})...');
      WaitCounter := 0;
      ...
      begin
        Log('Zora mutex still held after timeout; force-killing remaining processes.');
        { Kill by image name. {#MyAppExeName} (e.g. warp.exe, dev.exe) is unique
          ...
          rely on Zora's Job Object (JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE) to
        ...
        Log('Zora has exited; proceeding with file installation.');
    end;
  end;

  { After a successful install, write a helper script for running the Zora CLI. }
  { We use this to add a "warp-" prefix (e.g. "warp-preview.cmd" vs. "preview.exe") }
  if CurStep = ssPostInstall then begin
    { Add Zora to PATH if requested }
