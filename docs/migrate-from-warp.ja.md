...
MCP 設定など)を以前のインストールから Zora へ引き継ぎたい方向けです。

移行元として想定されるのは 2 つあり、**両者では安全性プロファイルが異なる**ため、
本書では別々のセクションで扱います。両方該当する場合は、**まず OpenWarp から
移行してから** Warp の移行を検討してください。

1. **OpenWarp** —— Zora の以前の名称。
2. **上流の [Warp](https://github.com/warpdotdev/warp)** —— Zora が fork して
   いるプロジェクト。
...
Zora(および OpenWarp / 上流 Warp も同様)はディスク上の状態を **3 種類の
ディレクトリ**に分けて格納します:
...
OpenWarp は改名前の Zora そのものです。改名コミット
(`feat: rename project Warp/OpenWarp → Zora`)は識別子とディスク上のパス名を
...
> コピー前に **Zora を終了**してください。プロセスがファイルを掴んでいると
> 失敗します。
...
`[ ! -e ... ]` / `-not (Test-Path $to)` のガードは、Zora 側で既にカスタマイズ
した内容を上書きしないためのものです。OpenWarp の値で上書きしたい場合は外して
ください。

Zora が問題なく動くことを確認したら、上記の OpenWarp ディレクトリを削除して
...
ます(上の「上流 Warp のソースパス」表を参照)。Zora はビルド時の channel が
`Oss` で、独自の app id(`\zap\Zap`)と、プラットフォームごとに分かれた
レイアウトを持ちます。両者のインストールは互いに相手のファイルを見ることが
できません —— これは Warp のアカウントやクラウド状態を Zora に持ち込まない
ための仕様でもあります。

下記のテキスト形式ファイルは schema が安定しており、コピーしても安全です。
**それ以外はそうではありません** —— Warp は Zora とは独立に進化しており、
...
- **`user_preferences.json`** —— macOS の
  ...
  ごとコピーすると ID 情報が漏れたり、Zora の認証状態が壊れたりします。
  Zora のデフォルトはもともとプライバシー優先なので、**触らないでください**。
- **`warp.sqlite`**(および `-wal` / `-shm` のサイドカー)—— schema が上流
  Warp に結合しており、Zora の migration が通る保証はありません。
- **Keychain / DPAPI / libsecret のエントリ** —— Warp の bundle / service 名に
  紐付いており、Zora からは利用できません。

### 手順

> コピー前に **Warp と Zora を終了**してください。
