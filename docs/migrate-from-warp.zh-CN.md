...
1. **OpenWarp** —— Zora 自己之前的名字。
2. **上游 [Warp](https://github.com/warpdotdev/warp)** —— Zora 所 fork 的项目。
...
Zora(以及 OpenWarp / 上游 Warp)把磁盘状态分成**三类目录**:
...
OpenWarp 就是改名前的 Zora。改名提交(`feat: rename project Warp/OpenWarp → Zora`)
...
`[ ! -e ... ]` / `-not (Test-Path $to)` 这层守卫是为了避免覆盖你已经在 Zora
里改过的内容。如果你就是想让 OpenWarp 的值覆盖掉 Zora,去掉它即可。

确认 Zora 一切正常之后,可以删掉上面那些 OpenWarp 目录来回收空间。它们已经不会
被任何程序使用了。
...
Zora 编译时 channel = `Oss`,对应独立的 app id(`\zap\Zap`)和按平台分开的
目录布局。两边互相看不到对方的文件 —— 这也正是 Zora 能让你的 Warp 账号 / 云端
状态留在 Warp 那边的原因。
...
- **`user_preferences.json`** —— 这是私有存储,位于 macOS 上的
  ...
  拷过去会泄漏身份信息,也会让 Zora 误判登录状态。Zora 默认值本身已经是隐私
  优先的,**不要碰它**。
- **`warp.sqlite`**(以及 `-wal` / `-shm` 伴生文件)—— schema 与上游 Warp 耦合,
  不保证能跑 Zora 的 migrations。
...
启动 Zora,你应该能在主题选择器里看到自定义主题,在快捷键编辑器里看到自定义
...
本文里的操作**都不是破坏性的**:拷过去的每个文件都是 Zora 启动时能自动用默认
值重建的。整体回滚:
