...
impl MCPServersListPageView {
    ...
    fn handle_server_card_event(&mut self, event: &ServerCardEvent, ctx: &mut ViewContext<Self>) {
        match event {
            ...
            ServerCardEvent::Share(item_id) => {
                log::debug!("Zora: MCP sharing is disabled for {item_id:?}");
            }
    ...
    fn file_based_root_chip_text(root_path: &PathBuf) -> Option<String> {
        ...
        // If the path is the Zora data directory (e.g. ~/.warp or ~/.warp_dev), set the text to
        // "global". The Zora provider stores its data directory as the root path rather than the
