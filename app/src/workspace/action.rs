...
pub enum WorkspaceAction {
    ...
    DropTab,
    /// Toggles the left panel. In Code Mode V1 this toggles Zora Drive.
    /// In Code Mode V2 this toggles the left panel which contains both the project explorer and
    /// Zora Drive. This happens as explicit action from the user.
    ToggleLeftPanel,
    /// Toggles directly to the Zora Drive tab of the left panel in Code Mode V2
    ToggleWarpDrive,
    /// Unconditionally opens Zora Drive. This is used in the case of user lifecycle
    ...
    OpenLink(String),
    /// On WASM, opens a given URL in the desktop Zora app (if installed) or redirects to download page.
    ...
    TerminateApp,
    CloseWindow,
    /// Help the user call the Zora executable with the [`crate::args::DEBUG_DUMP_FLAG`].
    DumpDebugInfo,
    ...
    // Zora Wave 7-3:`OpenEnvironmentManagementPane` WorkspaceAction 随 ambient-agent UI
    // 子系统物理删。
    ...
    /// Install the Zora CLI command to /usr/local/bin
    #[cfg(target_os = "macos")]
    InstallCLI,
    /// Uninstall the Zora CLI command from /usr/local/bin
    #[cfg(target_os = "macos")]
    UninstallCLI,
    ...
    /// Open the Zora Launch Modal (for debugging)
    #[cfg(debug_assertions)]
    OpenZapLaunchModal,
