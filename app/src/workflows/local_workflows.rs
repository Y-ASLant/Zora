...
impl LocalWorkflows {
    ...
    /// Returns an iterator over hardcoded "application" workflows included in the Zora binary.
    pub fn app_workflows(&self) -> impl Iterator<Item = &Workflow> {
        self.app_workflows.iter()
    }

    /// Returns an iterator over the static set of workflows for 3rd party tools loaded from Zora's
