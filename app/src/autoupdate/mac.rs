...
pub(super) fn relaunch() -> Result<()> {
    ...
    // 启动新版 Zora 前先等待当前进程退出，避免 Dock 中短暂出现多个图标。
...
/// Determines if the user needs authorization in order to update Zora.
...
/// * Zora can create files in the directory
/// * Zora can modify the permissions of created files
...
/// team identifier matches Zora's team identifier.
...
async fn apply_update(channel: Channel, version_info: &VersionInfo, update_id: &str) -> Result<()> {
    ...
    if async_fs::metadata(executable_temp_file.as_path())
        ...
        .is_ok()
    {
        // If we performed this process already but didn't relaunch Zora, the old executable will
...
async fn mount_dmg(dmg_dir: &Path, update_id: &str) -> Result<PathBuf> {
    ...
    // -nobrowse: Do not show the Zora DMG in Finder or similar apps.
    // -noautoopen: Do not open the Zora DMG in Finder.
...
fn dmg_name(channel: Channel) -> String {
    ...
    // openWarp GitHub Release 资产名固定使用 `Zora-arm64.dmg` / `Zora-intel.dmg`
    // (来自 .github/workflows 的命名约定),与 OSS 的内部 channel 名称不一致。
    // 这里只对 OSS 写死,不会影响官方 channel 的 universal 命名。
    if matches!(channel, Channel::Oss) {
        return if is_arm64 {
            "Zora-arm64.dmg".to_string()
        } else {
            "Zora-intel.dmg".to_string()
        };
    }
