...
pub struct PromptConfiguration {
    ...
    /// The separator to use as a trailing character at the end of Zora prompt, if any.
...
impl Prompt {
    ...
    /// Reset to the default Zora prompt.
    ...
    /// Whether same line prompt is enabled for Zora prompt.
    pub fn same_line_prompt_enabled(&self) -> bool {
        self.config.same_line_prompt_enabled
    }

    /// The separator to be used for the Zora prompt.
    pub fn separator(&self) -> WarpPromptSeparator {
...
impl PromptConfiguration {
    /// The default Zora prompt, synthesized from legacy prompt settings.
