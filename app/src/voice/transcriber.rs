...
pub enum TranscribeError {
    ...
    #[error("Zora is currently overloaded. Please try again later.")]
    ServerOverloaded,
    ...
    /// Zora 已禁用语音转写(BYOP genai 协议无法承载音频)。
    #[error("Voice transcription is unavailable in Zora.")]
    Disabled,
...
impl VoiceTranscriber {
    ...
    /// Zora(本地化,Phase 4):创建一个禁用的 transcriber。原语义上 `Some(...)`
