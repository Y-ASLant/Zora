// Zora:telemetry 发送层与 context provider 已删除。
...
// Compatibility metadata for local Zora Drive object event shells.
...
/// Common metadata retained for local Zora Drive event call sites that act on a specific object.
...
pub struct ObjectTelemetryMetadata {
    ...
    /// Legacy server UID slot. Zora keeps it optional while object-event call sites are being
    /// localized.
...
// Zora Phase 2a: `OpenedSharingDialogEvent` + `SharingDialogSource` and
...
pub enum TelemetryEvent {
    ...
    /// The native app was opened while logged out. Since Zora requires login,
    ...
    /// Opened legacy Zora AI.
    OpenedWarpAI {
        source: OpenedWarpAISource,
    },
    /// Issued legacy Zora AI request.
    WarpAIRequestIssued {
    ...
    // Toggled the legacy Zora AI side panel.
    ToggleWarpAI {
        opened: bool,
    },
    ...
    /// This is a telemetry event used to help track performance of Agent Predict in Zora,
    ...
    AgentModePrediction {
        ...
        // Zora leaves these optional; no telemetry sender consumes them.
    ...
    PromptSuggestionShown {
        ...
        /// suggestion. Zora keeps it optional for local diagnostics only.
    ...
    SuggestedCodeDiffBannerShown {
        ...
        block_id: Option<String>,
        request_duration_ms: u64,
        /// Legacy request token from the `/passive-suggestion` request. Zora keeps it optional
    ...
    /// Detected that Zora is running in an isolated sandbox.
