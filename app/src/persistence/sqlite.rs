...
unsafe fn init_logging() {
    ...
    extern "C-unwind" fn log_callback(_data: *mut c_void, err_code: c_int, msg: *const c_char) {
        ...
        let level = match (primary_error_code, err_code) {
            ...
            // This is mostly outside of Zora's control (e.g. the user or some system program is
    ...
    if warp_core::channel::ChannelState::channel() == warp_core::channel::Channel::Oss {
        if let Some(legacy_dir) = zap_legacy_app_group_sqlite_dir() {
            if let Err(err) = migrate_zap_app_group_sqlite_if_needed(&db_path, &legacy_dir)
                .context("Failed to migrate Zora SQLite database out of legacy App Group")
            {
                report_error!(err);
...
fn migrate_zap_app_group_sqlite_if_needed(target_db: &Path, legacy_dir: &Path) -> Result<()> {
    ...
    safe_info!(
        safe: ("Migrated Zora SQLite database out of legacy App Group"),
        full: ("Migrated Zora SQLite database from `{}` to `{}`", legacy_db.display(), target_db.display())
    );
...
fn save_pane_state(
    ...
    match &snapshot.contents {
        ...
        // Zora Wave 7-3:`EnvironmentManagement` LeafContents arm 随 variant 一同物理删。
