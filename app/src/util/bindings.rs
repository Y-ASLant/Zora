...
pub enum CustomAction {
    ...
    CopyBlockCommand,
    CopyBlockOutput,
    // Zora Wave 6-8:`ViewSharedBlocks` 随 `ShowBlocksView` 设置页与
    ...
    #[cfg(windows)]
    WindowsCopy,
    /// Also applies to legacy Zora AI (toggles the panel)
    NewAgentModePane,
    /// Also applies to legacy Zora AI (attaches the selection to the panel editor)
...
lazy_static! {
    ...
    /// know about particular Zora specific actions, so it deals with all actions
    /// as plain isizes.  Within Zora though we want to deal with them as the enum type.
