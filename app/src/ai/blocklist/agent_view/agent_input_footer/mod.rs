pub(super) mod chips;
pub mod editor;
// Zora Wave 7-3:`environment_selector` was removed with the hosted-mode footer.
...
// Zora Wave 7-3:`warp_cli::agent::Harness` import was removed with the hosted-mode footer.
...
// Zora Wave 7-3:`EnvironmentSelector` / `EnvironmentSelectorEvent` re-export was removed
...
// Zora Wave 7-3: removed the hosted-mode footer gap constant with the old footer.
...
pub struct AgentInputFooter {
    ...
    // Zora Wave 7-3:`environment_selector` field was removed with the hosted-mode footer.
...
impl AgentInputFooter {
    ...
    pub fn new(
        ...
        // Zora Wave 7-3:`EnvironmentSelector` 初始化 + 订阅 + ambient_agent
        ...
        let mut me = Self {
            ...
            // Zora Wave 7-3:`environment_selector` field init was removed with hosted-mode UI.
            // 子系统物理删。
    ...
    // Zora Wave 7-3: hosted-mode footer rendering was removed.
        ...
        Fut: std::future::Future<Output = Result<(), PluginInstallError>> + Send + 'static,
    {
        ...
        let (shell_path, shell_type) = match shell_data {
            ...
            // needing the Zora plugin to integrate with it.
    ...
    pub fn has_open_chip_menu(&self, app: &AppContext) -> bool {
        // Zora Wave 7-3:`environment_selector` is_menu_open() check was removed with hosted-mode UI.
        // 子系统物理删。
    ...
    pub fn toggle_cli_voice_input(
        ...
        match &self.cli_voice_input_state {
            CLIVoiceInputState::Stopped => {
                // Zora(Phase 3c A1):删除 `AIRequestUsageModel::can_request_voice`
...
impl View for AgentInputFooter {
    ...
    fn render(&self, app: &warpui::AppContext) -> Box<dyn warpui::Element> {
        ...
        // Zora Wave 7-3: removed environment-selector ambient-agent chip injection.
...
pub enum AgentInputFooterEvent {
    ...
    // Zora Wave 7-3:`OpenEnvironmentManagementPane` event was removed with hosted-mode UI.
    // 物理删。
...
/// Green-accented theme for the "Install Zora plugin" chip.
...
async fn write_install_log(agent: CLIAgent, err: &PluginInstallError) -> Option<PathBuf> {
    ...
    let contents = format!(
        "Zora plugin installation — {agent:?}\n\
         {now}\n\
         \n\
         {log}",
        log = err.log,
