...
fn test_harness_filter_matches_only_selected_harness() {
    App::test((), |mut app| async move {
        ...
        // Snapshot present but no harness set → Some(Oz), matches Zora Agent.
        ...
        // Local conversation: effectively Zora Agent.
        ...
        app.update(|ctx| {
            ...
            // Zora Agent / Oz → default-snapshot task and local conversation.
            ...
            assert_eq!(
                oz_items.len(),
                2,
                "expected 2 Zora Agent matches, got {oz_items:?}"
            );
            ...
            assert!(
                ...
                "stub task with no snapshot should not match the Zora Agent filter"
            );
        });
    });
}
