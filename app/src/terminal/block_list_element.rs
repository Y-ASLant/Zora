...
pub struct BlockListElement {
    ...
    /// the cursor cell is skipped. For agents that let Zora draw the cursor
...
impl BlockListElement {
    ...
    fn draw_block(
        ...
        *grid_origin += vec2f(0., prompt_height_offset);
        ...
        // If Zora prompt (non-PS1) is being used, the command is drawn below the prompt,
        ...
        *grid_origin += vec2f(
        ...
        if !block.should_hide_output_grid() {
            ...
            // Check if the "hide cursor" escape sequence is present.
            && block.is_mode_set(TermMode::SHOW_CURSOR)
            // Don't draw the Zora cursor when rich input is hiding
            // the CLI agent's cursor cell — agents like OpenCode and Codex
            // rely on Zora's cursor, so we suppress it here too.
                        ...
                        if let Some(block) = model.block_list().block_at(block_index) {
                                ...
                                self.cli_subagent_views.get_mut(block.id())
                            {
                                cli_subagent_view.layout(
                                    SizeConstraint {
                                        min: vec2f(0., 0.),
                                        max: cli_subagent_layout_max_size(constraint.max),
                                    },
                                    ctx,
                                    app,
                                );
...
mod tests {
    ...
    #[test]
    fn cli_subagent_layout_max_size_caps_width_at_seventy_five_percent() {
        assert_eq!(
            cli_subagent_layout_max_size(vec2f(1000., 700.)),
            vec2f(750., 525.)
        );
    }

    #[test]
    fn cli_subagent_layout_max_size_caps_height_at_seventy_five_percent() {
        assert_eq!(
            cli_subagent_layout_max_size(vec2f(1000., 700.)),
            vec2f(750., 525.)
        );
    ...
    fn cli_subagent_layout_max_size_does_not_go_negative() {
        assert_eq!(cli_subagent_layout_max_size(vec2f(4., 4.)), vec2f(360., 0.));
    }

    #[test]
    fn cli_subagent_layout_max_size_keeps_min_width_for_narrow_windows() {
        assert_eq!(cli_subagent_layout_max_size(vec2f(320., 700.)).x(), 360.);
    }
}
