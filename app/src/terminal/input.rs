...
pub enum CommandExecutionSource {
    /// A non-shared command execution request from Zora AI++.
...
impl Input {
    ...
    pub(crate) fn new(
        ...
        ctx.subscribe_to_view(&agent_input_footer, |me, _, event, ctx| {
            match event {
                ...
                // Zora Wave 7-3:`AgentInputFooterEvent::OpenEnvironmentManagementPane` handler
    ...
    fn agent_mode_hint_text(&mut self, app: &AppContext) -> String {
        ...
        let key = match (
            ...
            (InputType::AI, _) => {
                ...
                //  * `None` (no conversation, empty, passive, or untitled) => new conversation => "Zora anything"
    ...
    /// Predicts the next action using an AI model and past context on blocks within Zora.
    ...
    fn env_vars_command_prefix(&self, env_vars_id: &SyncId, ctx: &AppContext) -> Option<String> {
        ...
        if shell_type == ShellType::Fish {
            // Zora currently doesn't support newlines in Fish, just prepend the vars
    ...
    fn can_attach_on_filepaths_paste_or_dragdrop(&self, ctx: &mut ViewContext<Self>) -> bool {
        // Shared session viewers cannot attach images in Zora.
        let is_viewer = self.model.lock().shared_session_status().is_viewer();
        if is_viewer {
            return false;
        }
    ...
    pub(crate) fn input_enter(&mut self, ctx: &mut ViewContext<Self>) {
            ...
            && self.ai_input_model.as_ref(ctx).is_ai_input_enabled()
        {
                ...
                .is_configuring_ambient_agent()
            {
                ...
                // Collect pending files for the spawn request. Cloud mode image attachments are disabled in Zora.
