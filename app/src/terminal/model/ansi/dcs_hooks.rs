...
/// Indicates that the following JSON-encoded message is hex-encoded for Zora's lifecycle hooks.
...
/// Indicates that the following JSON-encoded message is unencoded for Zora's lifecycle hooks.
/// In DCS, it is used as the final char in the DCS start sequence.
/// In OSC, it is used as the first parameter.
pub(super) const UNENCODED_JSON_MARKER: char = 'f';

/// Indicates that the following message is a ANSI-C quoted message for receiving Zora's lifecycle
/// hooks via key-value pairs.
/// In OSC< it is used as the first parameter.
pub(super) const UNENCODED_KV_MARKER: char = 'k';

/// Enum representing all possible JSON payloads for Zora's DCS's.
...
/// Received from the pty after the shell has finished executing Zora's
/// bootstrap script.
...
/// Received from the pty when Zora's SSH wrapper is executed, prior to
...
/// created; if the session is for a subshell, this triggers Zora's bootstrap process.
...
/// The binding is triggered when Zora writes the `ESC-i` escape sequence to the pty.
/// Zora usually does this after a block completes, to collect any typeahead
...
/// `logout`, Ctrl-D on an empty prompt, etc.). Lets the Zora client drop
