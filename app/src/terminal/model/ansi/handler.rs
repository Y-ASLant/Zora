...
pub trait Handler {
    ...
    /// Callback for the Zora CommandFinished hook.
    ...
    /// Callback for the Zora precmd hook.
    fn precmd(&mut self, _data: PrecmdValue) {}

    /// Callback for the Zora preexec hook.
    fn preexec(&mut self, _data: PreexecValue) {}

    /// Callback for the Zora bootstrapped hook - called once when the shell is
    /// bootstrapped
    fn bootstrapped(&mut self, _data: BootstrappedValue) {}

    /// Callback for the Zora pre-interactive SSH session hook - called once
    ...
    /// Callback for the Zora ssh hook - called once after successfully connecting to
    /// an SSH server
    ...
    /// Callback for the Zora exit-shell hook — emitted by the remote shell right
    /// before it exits. Gives the Zora client a chance to drop per-session
    ...
    /// input buffer (the reporting is itself triggered by Zora).
    ...
    /// created. If the session is for a subshell, this should triggers Zora's bootstrap process.
    ...
    /// These allow external programs to trigger notifications in Zora.
