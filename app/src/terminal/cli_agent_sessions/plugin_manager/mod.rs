...
/// Manages the Zora notification plugin for a specific CLI agent.
///
...
pub(crate) trait CliAgentPluginManager: Send + Sync {
    /// The minimum plugin version required by this Zora build.
    ...
    /// Whether the Zora notification plugin is installed.
    ...
    /// Install the Zora notification plugin.
    ...
    /// Update the Zora notification plugin to the latest version.
    /// Default returns an error — only agents with `can_auto_install() == true` should override.
...
/// doesn't have Zora notification plugin support.
