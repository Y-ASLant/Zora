...
//!
//! This module owns everything specific to running a Zora shell inside a
...
///
/// Lives under the Zora per-user cache directory rather than `/tmp` so:
...
/// Resolves the absolute path to the `sbx` CLI binary using the Zora
/// process's `PATH`.
///
/// Zora's process `PATH` is minimal and often misses user-shell-installed
/// tools (e.g. homebrew on Apple Silicon when Zora is launched from Finder,
...
///
/// Each instance carries a unique `sandbox_id` so multiple Zora panes can run
...
impl DockerSandboxShellStarter {
    ...
    /// Host directory where Zora writes this sandbox's bash init script.
