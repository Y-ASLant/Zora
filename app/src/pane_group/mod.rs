...
// Zora:删除 ShareSessionModal import(云端 shared session 弹窗)
...
// Zora Wave 7-3:`EnvironmentManagementPane` 随 ambient-agent UI 子系统物理删。
...
///    MSYS2/WSL on Windows) that a raw `PATH` lookup would miss when Zora is
...
pub enum Event {
    ...
    /// tell the workspace to open a file within Zora.
    OpenFileInWarp {
    ...
    OpenAddRulePane,
    // Zora Wave 7-3:`OpenEnvironmentManagementPane` event 随 ambient-agent UI
    // 子系统物理删。
    OpenFilesPalette {
        source: PaletteSource,
    },
    ...
    /// Zora:在终端里 Ctrl/Cmd+点击远端 SSH 会话输出中的文件路径时发出,
...
pub struct PaneGroup {
    ...
    // Zora:删除 terminal_with_open_share_block_modal / share_block_modal 字段(云端 share block)
    dragged_border: Option<DraggedBorder>,
    user_default_shell_changed_banner: ViewHandle<Banner<PaneGroupAction>>,

    // Zora:删除 terminal_with_open_share_session_modal / share_session_modal 字段(云端 shared session)
    ...
    // Zora Wave 7-3:`pane_with_open_environment_setup_mode_selector` /
...
impl PaneGroup {
    ...
    fn restore_pane_leaf(
        ...
        let result = match leaf.contents {
            ...
            } // Zora Wave 7-3:`EnvironmentManagement` LeafContents arm 随 ambient-agent UI
              // 子系统物理删。
        };
    ...
    /// is used for intra-session prompt switching between Zora prompt and PS1.
    ...
    fn open_share_session_denied_modal(
        ...
        // Zora:share_session_modal 已删,no-op
        ctx.notify();
    }
    ...
    // Zora:删除 handle_share_session_modal_event(云端 shared session 弹窗)
    ...
    fn new_internal(
        ...
        // Zora:删除 share_block_modal 注册(云端 share block)
            ...
            Banner::<PaneGroupAction>::new_permanently_dismissible(
                BannerTextContent::formatted_text(vec![
                    FormattedTextFragment::plain_text(
                        "Zora doesn't currently support your default shell, falling back to zsh.  ",
                    ),
        ...
        // Zora:删除 share_session_modal 注册(云端 shared session 弹窗)
        ...
        let mut pane_group = Self {
            ...
            // Zora Wave 7-3:ambient-agent UI 子系统中的 pane-level modal 跟踪
            // 字段随 UI 物理删。
    ...
    fn load_data_into_transcript_viewer(
        ...
        // Insert the conversation ended tombstone (includes Open in Zora button on WASM).
    ...
    // Zora:删除 handle_share_block_modal_event(云端 share block)
    ...
    pub fn close_pane(&mut self, pane_id: PaneId, ctx: &mut ViewContext<Self>) {
        ...
        if FeatureFlag::UndoClosedPanes.is_enabled() {
            ...
            // Zora:删除 share_block_modal cleanup(云端 share block)
            // Zora Wave 7-3:ambient-agent UI 子系统中的 pane-level modal 跟踪
            // 字段 cleanup 随 UI 物理删。
    ...
    pub fn close_overlays(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        // Zora:删除 terminal_with_open_share_block_modal 清空(字段已不存在)
        ctx.notify();
    }
...
impl View for PaneGroup {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // Zora:删除 share_block_modal / share_session_modal / role-change modal 渲染分支(字段已不存在)
