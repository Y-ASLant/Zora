...
pub enum WasmNUXDialogAction {
    ...
    OpenNativeAndClose,
    /// Open the Zora download page
    OpenDownloadDesktopAppLink,
    /// Open a link to learn more about Zora
    LearnMore,
}
...
/// * Download Zora if they haven't already
...
impl WasmNUXDialog {
    ...
    /// behavior: if Zora is installed, redirect to it; otherwise stay on the web.
...
impl View for WasmNUXDialog {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // 1. The user doesn't have Zora installed - treat them as a potential new user and encourage downloading Zora.
        // 2. The user has Zora installed, but clicked through to the web - ask if they want to always default to web.
...
impl TypedActionView for WasmNUXDialog {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            WasmNUXDialogAction::OpenNativeAndClose => {
                // We intentionally do not set the native preference here, in case the user hasn't actually installed Zora.
                // If they have, on subsequent loads, we'll detect that Zora is installed and redirect to the desktop.
                ctx.emit(WasmNUXDialogEvent::Close);
