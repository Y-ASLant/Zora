//! Zora 本地 managed-secrets client。
//!
//! Zora 上游这里原本通过 server_api 调云端接口维护团队/用户托管密钥。
//! Zora 保留 `warp_managed_secrets` crate 供本地功能复用,但所有云端托管密钥
...
impl ManagedSecretsClient for DisabledManagedSecretsClient {
    ...
    async fn create_managed_secret(
        ...
        Err(anyhow!("Cloud managed secrets disabled in Zora"))
    }
    ...
    async fn issue_task_identity_token(
        ...
        Err(anyhow!("Task identity token issuance disabled in Zora"))
    }
}
